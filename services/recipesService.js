...
export const getRecipeById = async (id, userId) => {
  const attributes = ["id", "title", "time", "description", "instructions", "thumb"];

  if (userId) {
    attributes.push([
      sequelize.literal(`EXISTS(SELECT 1 FROM user_favorites WHERE recipeid = :recipeid AND userid = :userid)`),
      'isFavorite'
    ]);
  }
